...
##' @param interleave if \code{TRUE} sunrise and sunset are returned
##' as an interleaved sequence of twilights, otherwise sunrise and
##' sunset times are returned separately.
...
##' @param pairs a dataframe of sunset, sunrise pairs each spanning a night.
##' @return If \code{interleave=FALSE}, \code{find.twilights}
##' returns a dataframe of sunset, sunrise pairs with columns
##' \item{\code{Sunset}}{times of sunset}
##' \item{\code{Sunrise}}{times of sunrise}
##' \item{\code{Midnight}}{time of midnight}
##' where each row corresponds to a night.  If \code{interleave=TRUE},
##' \code{find.twilights} return a dataframe with columns
##' \item{\code{Twilight}}{times of twilight}
##' \item{\code{Rise}}{logical indicating sunrise}
##' where each row corresponds to a single twilight.  Dataframes of
##' sunset, sunrise pairs can be converted to interleaved format with
##' \code{interleave.twilights}.
##' @export
find.twilights <- function(tagdata,threshold,include,
                           exclude=NULL,interleave=TRUE,extend=0,dark.min=0) {
  ...
  d <- data.frame(Sunset=ss,
                  Sunrise=sr,
                  Midnight=ss+(sr-ss)/2)
  if(interleave) interleave.twilights(d) else d
}


##' @rdname find.twilights
##' @export
interleave.twilights <- function(pairs) {
  data.frame(Twilight=.POSIXct(as.vector(t(cbind(pairs$Sunset,pairs$Sunrise))),"GMT"),
