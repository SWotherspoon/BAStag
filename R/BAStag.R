...
##' \code{read.act} function imports activity data from an early
##' generation BAS tag while \code{read.act2} imports activity data
##' from later generation BAS tags that run length encode activity.
##' If \code{read.act2} is passed a dataframe constructed with
##' \code{read.lig}, it interpolates activity to the times at which
...
##' @param d.lig dataframe of light data created with \code{read.lig}
##' @param skip number of initial lines to skip
##' @return \code{read.act} returns a dataframe with columns
##' \item{\code{Valid}}{an indicator of whether the record is valid}
##' \item{\code{Date}}{the date and time of the observation}
##' \item{\code{Julian}}{date and time as Julian date}
##' \item{\code{Activity}}{the recorded activity level}
##' \item{\code{Temp}}{the recorded temperature}
##' If \code{read.act2} is passed a \code{d.lig} dataframe, it returns that
...
##' @rdname read.act
##' @export
read.act2 <- function(file,d.lig=NULL,skip=0) {
  ## Read csv file and add column names.  Must explicitly specify
  ## column names as R cannot detect the correct number of columns to
  ## read
  d <- read.csv(file,header=FALSE,skip=0,
...
##'
##' The \code{read.tem2} function reads temperature data from a later
##' generation BAS tag.  If \code{read.tem2} is passed a dataframe
##' constructed with \code{read.lig}, it matches the recorded
...
##' \item{\code{Temperature}}{the recorded temperature.}
##' @export
read.tem2 <- function(file,d.lig=NULL,skip=0) {
  ## Read csv file and add column names.  Must explicitly specify
  ## column names as R cannot detect the correct number of columns to
  ## read
  d <- read.csv(file,header=FALSE,skip=0,
...
##' \code{interval.data} computes a summary statistic for each
##' interval. The \code{twilight.data} is similar, but the time
...
##' @export
interval.data <- function(date,y,start,end,
...
##' @rdname interval.data
##' @export
twilight.data <- function(date,y,twl,before=4,after=4,
                          FUN=function(y) if(all(is.na(y))) NA else mean(y,na.rm=TRUE)) {
  mapply(
    function(start,end) FUN(y[start <= date & date <= end]),
